const AIService = (() => {
  ...
  async function callClaude(system, userMessage, temperature = 0.8, maxTokens = 4096, taskType = 'general') {
    const MAX_RETRIES = 3;
    const RETRY_DELAYS = [2000, 4000, 8000];

    const getLangDirective = () => {
      if (typeof I18n === 'undefined') return '';
      const currentLang = I18n.getLang ? I18n.getLang() : 'vi';
      const directives = {
        vi: '\n\n[MANDATORY LANGUAGE DIRECTIVE]: Học viên đang học bằng Tiếng Việt. Bạn BẮT BUỘC phản hồi 100% bằng Tiếng Việt chuẩn mực, sư phạm, giải thích tận tình và dễ hiểu.',
        en: '\n\n[MANDATORY LANGUAGE DIRECTIVE]: The user is learning in English. You MUST respond 100% completely and naturally in English with high pedagogical clarity.',
        ja: '\n\n[MANDATORY LANGUAGE DIRECTIVE]: ユーザーは日本語で学習しています。必ず100%自然で正確、丁寧でわかりやすい日本語で回答してください。',
        ko: '\n\n[MANDATORY LANGUAGE DIRECTIVE]: 학습자는 한국어로 학습 중입니다. 반드시 100% 자연스럽고 정확하며 친절한 한국어로 답변해 주세요.',
        zh: '\n\n[MANDATORY LANGUAGE DIRECTIVE]: 学员正在使用中文学习。请务必使用100%通俗易懂、准确且自然的中文进行解答。',
        fr: '\n\n[MANDATORY LANGUAGE DIRECTIVE]: L apprenant étudie en français. Vous DEVEZ répondre à 100% en français clair et pédagogique.',
        de: '\n\n[MANDATORY LANGUAGE DIRECTIVE]: Der Lernende lernt auf Deutsch. Antworten Sie bitte zu 100% auf verständlichem und professionellem Deutsch.',
        es: '\n\n[MANDATORY LANGUAGE DIRECTIVE]: El estudiante está aprendiendo en español. DEBES responder 100% en español pedagógico y claro.'
      };
      return directives[currentLang] || directives.en;
    };
    const langDirective = getLangDirective();

    const effectiveSystem = system + langDirective;

    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      try {
        const authHdr = (typeof AuthService !== 'undefined' && AuthService.isConfigured())
        ...
        if (res.status === 401) {
          ...
          throw new Error(err.error || 'Bạn cần đăng nhập để dùng tính năng AI.');
        }
...
[{"question":"...","code":"int x = 5;\\nint y = 10;\\ncout << x + y;","options":["15","10","5","Lỗi"],"correct":0,"explanation":"..."}]`;
    ...
    const text = await callClaude(system, prompt, 0.85, 4096, 'quiz');
...
{"title":"...","description":"Mô tả chi tiết","hints":["gợi ý 1","gợi ý 2"],"sampleInput":"...","expectedOutput":"...","solution":"code solution","solutionLang":"python/cpp/java/etc"}`;
    ...
    const text = await callClaude(system, prompt, 0.85, 4096, 'exercise');
...
{"score":0-100,"status":"correct|partial|incorrect","feedback":"nhận xét tổng thể","errors":["lỗi nếu có"],"suggestions":["gợi ý cải thiện"],"correctedCode":"code sửa nếu cần"}`;
    ...
    const text = await callClaude(system, prompt, 0.3, 4096, 'checkAnswer');
    return parseJSON(text);
  }
...
NGUYÊN TẮC VÀNG: Mỗi lần phải tạo idea HOÀN TOÀN KHÁC BIỆT. TUYỆT ĐỐI KHÔNG lặp lại dạng "máy tính",  "calculator", hay bất kỳ pattern nào giống nhau.`;
    ...
    const langInstructions = lang === 'vi'
      ? 'Viết toàn bộ nội dung (tên, mô tả, yêu cầu, các bước, thời gian) bằng Tiếng Việt.'
      : `LANGUAGE MANDATE: Write ALL text fields ("name", "description", "requirements", "steps", "bonusFeatures", "estimatedTime") strictly in ${lang === 'en' ? 'ENGLISH' : lang === 'ja' ? 'JAPANESE' : lang === 'ko' ? 'KOREAN' : lang === 'zh' ? 'CHINESE' : lang === 'fr' ? 'FRENCH' : lang === 'de' ? 'GERMAN' : lang === 'es' ? 'SPANISH' : 'ENGLISH'}. Do NOT use Vietnamese.`;
    ...
    if (topic) {
      ...
      prompt = `Generate 1 creative and realistic programming project idea.
${langInstructions}

🎯 TOPIC (MANDATORY): "${topic}"
${level ? `DIFFICULTY LEVEL: ${level} — ${levelContext}` : 'DIFFICULTY LEVEL: auto-determined'}

💡 Technical Twist (optional): "${twist}"

⚠️ RULES:
1. The project MUST strictly match the topic "${topic}"
2. Seed: ${seed}-${rndNum}
3. ${langInstructions}

Return strictly JSON:
{
  "name": "Concise creative project name",
  "description": "Detailed project description (3-5 sentences)",
  "difficulty": "${level || 'mid'}",
  "techStack": ["recommended technologies"],
  "requirements": ["requirement 1", "requirement 2", "..."],
  "steps": ["step 1: ...", "step 2: ...", "..."],
  "bonusFeatures": ["bonus features for extra credit"],
  "estimatedTime": "${lang === 'vi' ? '6-10 giờ' : '6-10 hours'}"
}`;
    } else {
      prompt = `Generate 1 creative, practical, and difficulty-appropriate programming project idea.
${langInstructions}

DIFFICULTY LEVEL: ${level} — ${levelContext}

🎯 DOMAIN FOR THIS PROJECT: "${randomCategory}"

⚠️ RULES:
- The project MUST be within the domain "${randomCategory}"
- Do NOT generate simple calculator/converter unless requested
- The project MUST fit the ${level} difficulty level
- Seed: ${seed}-${rndNum}
- ${langInstructions}
...
{
  "name": "Concise creative project name",
  "description": "Detailed project description (3-5 sentences) explaining why it is impactful",
  "difficulty": "${level}",
  "techStack": ["recommended technologies suitable for level"],
  "requirements": ["functional requirement 1", "requirement 2", "requirement 3", "..."],
  "steps": ["step 1: ...", "step 2: ...", "step 3: ...", "..."],
  "bonusFeatures": ["bonus features for advanced learning"],
  "estimatedTime": "${lang === 'vi' ? '6-10 giờ' : '6-10 hours'}"
}`;
    }
...
${projectContext}`;
    ...
    return await callClaude(system, conversationContext + userMessage, 0.7, 4096, 'projectChat');
  }
...
${lessonContext}`;
    ...
    return await callClaude(system, conversationContext + userMessage, 0.7, 4096, 'tutor');
  }
  ...
  function groupFilesIntoModules(files) {
    ...
    for (const f of files) {
      ...
      if (!placed) categories.logic.files.push(f);
    }
...
Trả về JSON:
{
  ...
  "positives": ["điểm tốt 1", "điểm tốt 2"]
    ...
    const text = await callClaude(system, prompt, 0.3, 4096, 'projectReview');
    return parseJSON(text);
  }
...
Trả về JSON:
{
  ...
  "tips": ["gợi ý nâng cao 1", "..."]
    ...
    const text = await callClaude(system, prompt, 0.3, 8192, 'projectReview');
    return parseJSON(text);
  }
